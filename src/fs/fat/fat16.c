...
int fat16_resolve(struct disk *disk)
{
    ...
    if (diskstreamer_read(stream, &private->header, sizeof(private->header)) != ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    if (diskstreamer_seek(stream, fat_table_pos * (cluster * FAT16_FAT_ENTRY_SIZE)) < 0)
        return -EIO;
...
int fat16_seek(void *private, uint32_t offset, FILE_SEEK_MODE mode)
{
    ...
    case FILE_SEEK_CUR:
        ...
        break;
    case FILE_SEEK_END:
        return -EUNIMP; // TODO implement in the future
        break;
    default:
        return -EINVARG;
        break;
