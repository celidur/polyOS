...
static int fat16_add_cluster_to_chain(struct fat_private *private, int cluster, int new_cluster)
{
    struct disk_stream *stream = private->fat_write_stream;
    if (!stream)
        return -EIO;
    ...
    if (next_cluster != FAT16_UNUSED && next_cluster < FAT16_BAD_SECTOR)
        return fat16_add_cluster_to_chain(private, next_cluster, new_cluster);

    u32 fat_table_pos = fat16_get_first_fat_sector(private) * private->sector_size;
    if (disk_streamer_seek(stream, fat_table_pos * (cluster * FAT16_FAT_ENTRY_SIZE)) < 0)
        return -EIO;

    if (disk_streamer_write(stream, &new_cluster, sizeof(new_cluster)) < 0)
        return -EIO;

    fat16_set_end_of_chain(private, new_cluster);    
...
static int fat16_get_free_cluster(struct fat_private *private)
{
    ...
    for (int i = 3; i < total_clusters; i++)
    {
...
static int fat16_get_cluster_for_offset(struct fat_private *private, int starting_cluster, int offset, bool create_new_cluster)
{
    ...
    for (int i = 0; i < cluster_ahead; i++)
    {
        ...
        if (entry >= FAT16_BAD_SECTOR || entry == FAT16_UNUSED){
            if (create_new_cluster && i == cluster_ahead - 1) {
                int new_cluster = fat16_get_free_cluster(private);
                if (new_cluster < 0)
                    return new_cluster;
                int res = fat16_add_cluster_to_chain(private, cluster, new_cluster);
                if (res < 0)
                    return res;
                return new_cluster;
            }
...
static int fat16_read_internal_from_stream(struct fat_private *private, struct disk_stream *stream, int cluster, int offset, void *buffer, int size)
{
    ...
    int cluster_use = fat16_get_cluster_for_offset(private, cluster, offset, false);
    if (cluster_use < 0)
        return cluster_use;
...
static int fat16_get_entry(struct fat_private *fat_private, struct fat_item *current_directory,int offset ,struct raw_data *entry) {
    ...
    if (current_directory->type == FAT_ITEM_TYPE_ROOT_DIRECTORY) {
        ...
        int root_directory_absolute_pos = fat16_sector_to_absolute(fat_private, root_directory_sector_pos);
        if (root_directory_absolute_pos + offset + sizeof(struct raw_data) > fat16_sector_to_absolute(fat_private, current_directory->root_dir->ending_sector_pos))
            return -ENOMEM;
        if (disk_streamer_seek(stream, root_directory_absolute_pos + offset) != ALL_OK)
            return -EIO;
...
// TODO: if end of cluster add new cluster
static int fat16_write_internal_to_stream(struct fat_private *private, struct disk_stream *stream, int cluster, int offset, void *buffer, int size)
{
    ...
    int cluster_use = fat16_get_cluster_for_offset(private, cluster, offset, true);
    if (cluster_use < 0)
        return cluster_use;
    ...
    if (size > 0)
        ...
        fat16_set_end_of_chain(private, cluster_use);
    return ALL_OK;
}
...
// For now it will not allocate more space in directory if not enough
static int fat16_get_free_item_entry(struct fat_private *private, struct fat_item *parent, int number) {
    if (!fat16_is_directory(parent))
        return -EINVARG;
    ...
    while (true)
    {
        int res = fat16_get_entry(private, parent, offset, &data);
        if (res < 0)
            return res;
        if (data.data[0] == 0x00 || data.data[0] == 0xE5)
        {
            if (nb_entries == 0)
                offset_start = offset;
            nb_entries++;
        ...
        if (nb_entries == number || data.data[0] == 0x00)
            return offset_start;
...
static int fat16_release_item_entry(struct fat_private *private, struct fat_item *parent, int offset, int number){
    if (!fat16_is_directory(parent))
        return -EINVARG;
    ...
    {
        u32 offset = offset + (i * sizeof(struct raw_data));
        u8 free_entry = FAT16_ENTRY_FREE;

        if (parent->type == FAT_ITEM_TYPE_ROOT_DIRECTORY) {
            int res = disk_streamer_seek(stream, root_directory_absolute_pos + offset);
            if (res < 0)
                return res;

            if (root_directory_absolute_pos + offset + sizeof(free_entry) > fat16_sector_to_absolute(private, parent->root_dir->ending_sector_pos))
                return -ENOMEM;
            ...
            serial_printf("Cluster: %d\n", fat32_get_first_cluster(&parent->directory->self));
            int cluster = fat32_get_first_cluster(&parent->directory->self);
...
static int fat16_update_fat_item_data(struct fat_private *private, struct fat_entry_t *item)
{
    ...
    {
        fat16_release_item_entry(private, item->parent, item->entry_offset, item->nb_entries);
    }
    
    if (item->nb_entries < nb_entries)
    {
        item->entry_offset = fat16_get_free_item_entry(private, item->parent, nb_entries);
        if (item->entry_offset < 0)
            return item->entry_offset;
    }
    ...
    if (item->parent->type == FAT_ITEM_TYPE_ROOT_DIRECTORY) {
        ...
        int res = fat16_write_internal(private, cluster, item->entry_offset, data, total_size);
        if (res < 0)
            return res;
    }
...
static int fat16_write(void *fd_private, void *descriptor, u32 size, void *in_ptr)
{
    ...
    if (cluster == 0)
    {
        ...
        fat16_set_end_of_chain(fd_private, cluster);
    }
...
static int fat16_remove(void *fs_private, struct path_part *path)
{
    struct fat_private *private = fs_private;
    struct fat_item *item = fat16_get_directory_entry(private, path);
    if (!item)
        return -EBADPATH;
    ...
    struct fat_entry_t *entry = item->item;
    int res = fat16_release_chain(private, fat32_get_first_cluster(entry));
    if (res < 0)
        return res;

    res = fat16_release_item_entry(private, item->item->parent, item->item->entry_offset, item->item->nb_entries);
    if (res < 0)
        return res;
